...
from hpa_core.dual_beam_mainline.constraints import build_constraint_assembly
from hpa_core.dual_beam_mainline.load_split import (
    build_dual_beam_load_split,
    collapse_explicit_rear_weight_to_equivalent_my_n,
    convert_torque_to_main_spar_reference,
)
from hpa_core.dual_beam_mainline.optimizer_view import (
    build_feasibility_summary,
    build_geometry_validity_margins,
    build_numerical_consistency_result,
    build_optimizer_facing_metrics,
)
from hpa_core.dual_beam_mainline.recovery import (
    build_report_metrics,
    recover_reactions,
    recover_structural_response,
)
from hpa_core.dual_beam_mainline.serialization import (
    load_dual_beam_mainline_model,
)
from hpa_core.dual_beam_mainline.smooth import build_default_smooth_scales
from hpa_core.dual_beam_mainline.solver import solve_dual_beam_state
...
TRACK_S_RERUN_SNAPSHOT_FIXTURE = (
    REPO_ROOT / "tests" / "fixtures" / "dual_beam_mainline" / "track_s_rerun_snapshot_model.json"
)


def _build_track_s_rerun_snapshot_model() -> DualBeamMainlineModel:
    """Load the captured Track S rerun snapshot kernel model.

    The fixture is a bit-exact serialization of the model the application layer
    produces. It is regenerated and verified in the application repository by
    `migration/capture_dual_beam_kernel_fixture.py`, which fails loudly on any
    field difference, so this suite never needs the application stack.
    """

    return load_dual_beam_mainline_model(TRACK_S_RERUN_SNAPSHOT_FIXTURE)
