"""Dependency-light kernel half of the dual-beam mainline public API.

This module holds the part of the analysis entry point that operates on an
already-built ``DualBeamMainlineModel``. It deliberately does not import
``builder``, so it stays inside the NumPy+SciPy kernel dependency boundary.
The application-facing wrapper lives in ``api``.
"""

from __future__ import annotations

from hpa_core.dual_beam_mainline.constraints import build_constraint_assembly
from hpa_core.dual_beam_mainline.load_split import build_dual_beam_load_split
...
from hpa_core.dual_beam_mainline.smooth import (
...
__all__ = ["run_dual_beam_mainline_kernel"]
